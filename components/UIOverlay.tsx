...
interface UIOverlayProps {
  ...
  onSendPhotosToTelegram?: () => Promise<void>;
}
...
export const UIOverlay: React.FC<UIOverlayProps> = ({
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const uploadPromises = uploadedPhotos.map(async (photo, index) => {
        ...
        uploadedCount++;
        setUploadProgress(
          `上传照片中 (${uploadedCount}/${uploadedPhotos.length})...`
        );
        return publicUrl;
      });
  ...
  const handleSendToTelegram = async () => {
    if (!onSendPhotosToTelegram || isSendingToTelegram) return;
    setIsSendingToTelegram(true);
    try {
      await onSendPhotosToTelegram();
    } finally {
  ...
  return (
    <div className="absolute top-0 left-0 w-full h-full pointer-events-none z-10">
      ...
      <div className="absolute bottom-8 right-8 flex flex-col items-end gap-4 pointer-events-auto">
        ...
        {!isSharedView && (
          <>
            {/* Upload Button - Always visible to allow adding more photos */}
            <button
